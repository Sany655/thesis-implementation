...
\subsection{3. Missing Value Imputation (KNN \& Median)}
\textbf{What:} We resolved missing entries in critical laboratory features (e.g., AST, ALT). \\
\textbf{How:} For the comprehensive pooled dataset, we applied Median Imputation to replace missing continuous variables, computed strictly from the training fold. For the highly constrained pediatric cohort (where AST/ALT sparsity was severe), we utilized K-Nearest Neighbors (KNN) Imputation ($k=5$, distance-weighted) to intelligently estimate missing values based on multivariate feature similarities. \\
\textbf{Why:} Clinical laboratory data is highly right-skewed, making the median more robust than the mean against outliers. In cases of severe sparsity within specific demographic subgroups (like pediatrics), KNN Imputation mathematically captures non-linear physiological relationships between available features (like Platelets and WBC) to accurately estimate missing liver transaminases without distorting the underlying distributions.

\subsection{4. Feature Scaling (Standardization)}
\textbf{What:} We normalized the scale of continuous variables (e.g., WBC, Age, HCT). \\
\textbf{How:} We applied a `StandardScaler` to transform features to have a mean of 0 and a standard deviation of 1, strictly fit on the training data. \\
\textbf{Why:} Many machine learning algorithms and distance-based metrics (like KNN and Logistic Regression) are sensitive to feature magnitudes. Scaling ensures that large-magnitude variables (like Platelet counts in hundreds of thousands) do not mathematically dominate small-magnitude variables (like Hemoglobin), ensuring unbiased gradient convergence and feature importance calculations.
...
\section{External Dataset Verification \& Comparative Analysis}
To address concerns of model overfitting and to rigorously evaluate the generalizability of the trained models, an external, highly-constrained pediatric dataset (Dengue Hematology Dataset bd v4, $n=158$) was extracted. 

Figure \ref{fig:pediatric_learning_curve} illustrates the learning curve of a new Random Forest model trained exclusively on this extracted cohort, demonstrating rapid convergence and stability (1.00 ROC-AUC) despite the limited sample size. 

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{../figures/pediatric/learning_curve.png}
    \caption{Learning Curve for the Random Forest model trained on the highly-constrained pediatric dataset, verifying rapid convergence without severe overfitting.}
    \label{fig:pediatric_learning_curve}
\end{figure}

Furthermore, the existing base \textit{pediatric\_best.pkl} model was benchmarked against this unseen v4 dataset to evaluate generalizability (Figure \ref{fig:model_comparison}). The base model exhibited a significant performance drop (ROC-AUC 0.53, Accuracy 35\%) when confronted with unseen demographic variations and imputed transaminase (AST/ALT) features. This stark contrast highlights the critical necessity of domain-specific model calibration and the severe limitations of deploying rigid legacy models across varying clinical populations.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{../figures/pediatric/model_comparison.png}
    \caption{Comparative performance evaluation: Existing legacy pediatric model vs. newly calibrated dataset-specific pipeline.}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{../figures/pediatric/shap_summary_examiner.png}
    \caption{SHAP Summary Plot for the newly calibrated Pediatric Dengue model (v4 cohort). This explicitly highlights the explainability of transaminase thresholds (AST, ALT) in driving model severity predictions for pediatric patients.}
    \label{fig:shap_examiner}
\end{figure}
